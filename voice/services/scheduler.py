...
from voice.models import Meeting, CallAttempt
from voice.constants import (
    LogLevel, CallPhase, CallStatus,
    PRE_MEETING_OFFSETS, POST_MEETING_OFFSETS
)
from voice.selectors import get_meetings_for_pre_call_check, get_meetings_for_post_call_check
...
def pre_program_meeting_calls(meeting: Meeting, force_recreate: bool = False) -> Dict[str, Any]:
    ...
    for phase, offset, scheduled_time in all_offsets:
        ...
        if existing:
            ...
            if existing.status == CallStatus.SCHEDULED:
                if existing.scheduled_time != scheduled_time:
...
def should_trigger_pre_call(meeting: Meeting, offset: int) -> bool:
    ...
    if existing_attempt:
        ...
        if offset == PRE_MEETING_OFFSETS[1]:  # -30 mins (retry)
            if existing_attempt.status in [CallStatus.NO_ANSWER, CallStatus.FAILED]:
                return True
        return False
    ...
    if offset == PRE_MEETING_OFFSETS[1]:
        ...
        if first_call_attempt and first_call_attempt.status == CallStatus.COMPLETED:
            return False
...
def should_trigger_post_call(meeting: Meeting, offset: int) -> bool:
    ...
    if existing_attempt:
        ...
        if existing_attempt.status == CallStatus.COMPLETED:
            return False
        # If attempt exists but failed/no answer, allow retry if it's the retry offset
        if offset == POST_MEETING_OFFSETS[1]:  # +30 mins (retry)
            if existing_attempt.status in [CallStatus.NO_ANSWER, CallStatus.FAILED]:
                return True
        return False
