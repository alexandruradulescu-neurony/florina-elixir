...
def create_or_update_deal(meeting: Meeting) -> dict[str, Any] | None:
    ...
    try:
        ...
        if existing_deal:
            ...
            response = client.put(f"{base_url}/deals/{deal_id}", json=deal_data)
...
def sync_note_to_pipedrive(deal_id: str | None, text: str, meeting: Meeting) -> dict[str, Any]:
    ...
    try:
        ...
        response = client.post(f"{base_url}/notes", json=note_data)
        ...
        if response.status_code in [200, 201]:
            ...
            if data.get("success"):
                ...
                log_activity(
                    ...
                    details={"deal_id": deal_id, "note_id": note_id, "note_length": len(text)},
                )
            else:
