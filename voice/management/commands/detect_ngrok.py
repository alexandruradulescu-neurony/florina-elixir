...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if validate_ngrok_url(ngrok_url):
            ...
            self.stdout.write(
                self.style.WARNING("[WARNING] URL does not match expected ngrok pattern")
            )
            ...
            if current_url != webhook_url:
                ...
                if options["update"]:
                    ...
                    if result.get("success"):
                        self.stdout.write(
                            self.style.SUCCESS("[OK] Webhook URL updated successfully!")
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(
                                f"[ERROR] Failed to update: {result.get('error', 'Unknown error')}"
                            )
                        )
                else:
