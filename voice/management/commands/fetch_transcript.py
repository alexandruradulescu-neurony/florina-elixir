...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--call-id", type=str, help="Specific call attempt ID to fetch transcript for"
        )
        parser.add_argument(
            "--external-call-id",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if call_id:
            ...
            if call:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Call attempt with external_call_id {external_call_id} not found"
                    )
                )
            ...
            self.stdout.write(
                f"Found {calls.count()} completed calls without transcripts in last {hours} hours"
            )
            self.stdout.write("")
            ...
            for call in calls:
                self.stdout.write(
                    f"Fetching transcript for call {call.id} (external_id: {call.external_call_id})..."
                )
            ...
            self.stdout.write(
                self.style.ERROR("Please specify --call-id, --external-call-id, or --all-pending")
            )
            self.stdout.write("\nUsage examples:")
    ...
    def _fetch_for_call(self, call, verbose=True):
        ...
        if not call.external_call_id:
            if verbose:
                self.stdout.write(
                    self.style.ERROR("No external_call_id available - cannot fetch from API")
                )
            return False
        ...
        if result:
            ...
            if verbose:
                ...
                self.stdout.write(
                    f"Transcript length: {len(call.transcript) if call.transcript else 0} chars"
                )
                if call.transcript:
                    preview = (
                        call.transcript[:200] + "..."
                        if len(call.transcript) > 200
                        else call.transcript
                    )
                    try:
