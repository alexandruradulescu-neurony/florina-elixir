...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        future_meetings = Meeting.objects.filter(
            start_time__gte=now, start_time__lte=now + timedelta(hours=2)
        )
        ...
        if future_meetings.count() > 0:
            ...
            for meeting in future_meetings:
                ...
                self.stdout.write(
                    f"  - {meeting.title} at {meeting.start_time} ({minutes_until} minutes from now)"
                )
                self.stdout.write(
                    f"    Agent: {meeting.agent.username}, Phone: {meeting.agent.phone_number or 'NOT SET'}"
                )
        ...
        for offset in PRE_MEETING_OFFSETS:
            ...
            self.stdout.write(
                f"  Call should be made when meeting starts at: {call_time_example} ± {SCHEDULER_WINDOW / 2} min"
            )
            self.stdout.write(
                f"  Looking for meetings starting between: {window_start} and {window_end}"
            )

            meetings_in_window = Meeting.objects.filter(
                start_time__gte=window_start, start_time__lte=window_end, agent__is_sales_agent=True
            )
        ...
        for meeting, offset in pre_calls:
            self.stdout.write(
                f"  - {meeting.title} (offset: {offset} min, start: {meeting.start_time})"
            )
        ...
        for meeting, offset in post_calls:
            self.stdout.write(
                f"  - {meeting.title} (offset: {offset} min, end: {meeting.end_time})"
            )
        ...
        for attempt in recent_attempts:
            self.stdout.write(
                f"  - {attempt.meeting.title} ({attempt.get_phase_display()}, {attempt.get_status_display()})"
            )
            self.stdout.write(
                f"    Offset: {attempt.scheduled_offset_minutes} min, Created: {attempt.created_at}"
            )
