...
class GoogleCalendarOAuthView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if ngrok_url:
            ...
            redirect_uri = config(
                "GOOGLE_REDIRECT_URI",
                default=request.build_absolute_uri(reverse("voice:google_calendar_callback")),
            )
        ...
        if "your-google-client-id" in client_id or client_id == "":
            ...
            messages.error(
                request,
                "Google Calendar Client ID is not properly configured. Please check your .env file.",
            )
            return redirect("voice:calendar_sync_status")
    ...
    def get(self, request):
        ...
        try:
            ...
            if ngrok_url:
                ...
                logger.info(
                    f"OAuth callback - Using ngrok URL for authorization_response: {authorization_response}"
                )
            else:
            ...
            log_activity(user=request.user, action="Google Calendar OAuth completed", level="INFO")
                ...
                if ngrok_url:
                    ...
                    if not webhook_url:
                        ...
                        webhook_url = request.build_absolute_uri(
                            reverse("voice:google_calendar_webhook")
                        )

                watch_result = setup_google_calendar_watch(
                    request.user, webhook_url, session=request.session
                )
...
class TestCallView(SuperuserRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if not phone_number:
            messages.error(
                request, "Please provide a phone number or set your phone number in your profile."
            )
            return redirect("voice:superuser_dashboard")
        ...
        if not validate_phone_number(phone_number):
            messages.error(
                request, "Invalid phone number format. Please use E.164 format (e.g., +1234567890)."
            )
            return redirect("voice:superuser_dashboard")
        ...
        try:
            ...
            if not prompt:
                messages.error(
                    request, "No active pre-meeting prompt found. Please create a prompt first."
                )
                return redirect("voice:superuser_dashboard")
...
class PromptEditView(SuperuserRequiredMixin, View):
    ...
    def get(self, request, prompt_id):
        ...
        if prompt.first_message:
            ...
            preview_first_message = preview_first_message.replace(
                "{customer_name}", "Acme Corporation"
            )
            preview_first_message = preview_first_message.replace(
                "{meeting_title}", "Q4 Product Demo"
            )
    ...
    def post(self, request, prompt_id):
        ...
        if prompt.is_active:
            VoicePrompt.objects.filter(prompt_type=prompt.prompt_type, is_active=True).exclude(
                id=prompt.id
...
class ProgrammedCallsView(SuperuserRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if show_upcoming:
            ...
            upcoming_calls.sort(
                key=lambda x: x.scheduled_time if x.scheduled_time else timezone.now()
            )
...
class MethodologyListView(SuperuserRequiredMixin, View):
    ...
    def get(self, request):
        methodologies = (
            Methodology.objects.all().select_related("created_by").order_by("-is_active", "name")
        )
        ...
        for m in methodologies:
            agents_using = User.objects.filter(is_sales_agent=True, default_methodology=m).count()
                    ...
                    if is_configured():
                        ...
                        if pdf_text:
                            ...
                            if summary:
                                ...
                                messages.success(
                                    request,
                                    f'Methodology "{methodology.name}" created with AI summary.',
                                )
                            else:
                                messages.warning(
                                    request,
                                    "Methodology created but AI summary generation failed. You can add it manually.",
                                )
                        else:
                            messages.warning(
                                request, "Methodology created but PDF text extraction failed."
                            )
                    else:
                        messages.info(
                            request,
                            "Methodology created. Configure ANTHROPIC_API_KEY to enable AI summarization.",
                        )
                except Exception as e:
...
class MethodologyEditView(SuperuserRequiredMixin, View):
    ...
    def get(self, request, methodology_id):
        ...
        return render(
            request,
            "voice/manager/methodology_form.html",
            {
                "form": form,
                    ...
                    if is_configured():
                        ...
                        if pdf_text:
                            ...
                            if summary:
                                ...
                                methodology.save(update_fields=["ai_summary"])
                                messages.success(
                                    request, "Methodology updated with new AI summary."
                                )
                                return redirect("voice:methodology_list")
                except Exception as e:
                    logger.error(f"Error processing methodology PDF: {e}", exc_info=True)
    ...
    def _build_context(self, visit, form):
        ...
        steps = [
            ...
            },
            {
                ...
                "label": "Meeting",
                "done": current_idx >= 2,
                "active": current_idx == 2,
            },
            {
                "key": "post_call",
                "label": "Post-Call",
                "done": current_idx >= 3,
                "active": current_idx == 3,
            },
            {
                "key": "complete",
                "label": "Complete",
                "done": current_idx >= 4,
                "active": current_idx == 4,
            },
        ]
...
class AgentMethodologyView(SuperuserRequiredMixin, View):
    ...
    def post(self, request, agent_id):
        ...
        if form.is_valid():
            ...
            msg = (
                f'Default methodology for {agent.username} set to "{methodology.name}".'
                if methodology
                else f"Default methodology cleared for {agent.username}."
            )
...
class ClientListView(SuperuserRequiredMixin, View):
    ...
    def get(self, request):
        ...
        with_summary = (
            Client.objects.filter(ai_summary__isnull=False).exclude(ai_summary="").count()
        )
...
class ClientEditView(SuperuserRequiredMixin, View):
    ...
    def get(self, request, client_id):
        ...
        return render(
            request,
            "voice/manager/client_form.html",
            {
                "form": form,
                "client": client,
                "editing": True,
